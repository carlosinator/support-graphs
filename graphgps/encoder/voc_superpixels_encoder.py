...
"""
Implementation adapted from https://github.com/toenshoff/LRGB
...
@register_node_encoder('VOCMaskedNode')
class VOCMaskedNodeEncoder(torch.nn.Module):
    def __init__(self, emb_dim):
        super().__init__()
...
@register_edge_encoder('VOCMaskedEdge')
class VOMaskedEdgeEncoder(torch.nn.Module):
    def __init__(self, emb_dim):
        super().__init__()
    ...
    def forward(self, batch):
        ...
        ea = batch.edge_attr
        ea[batch.real_edge_mask] = (ea[batch.real_edge_mask] - self.edge_x_mean.view(1, -1)) / self.edge_x_std.view(1, -1)
        ea[~batch.real_edge_mask] = 0
        batch.edge_attr = self.encoder(ea)
...
#####################################################
#
# COCO dataset encoders
#
#####################################################


@register_node_encoder('COCOMaskedNode')
...
@register_edge_encoder('COCOMaskedEdge')
