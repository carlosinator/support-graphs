...
def pyg_softmax(graphgps, index, num_nodes=None):
    r"""Computes a sparsely evaluated softmax.
    Given a value tensor :attr:`graphgps`, this function first groups the values
    ...
    out = graphgps - scatter_max(graphgps, index, dim=0, dim_size=num_nodes)[0][index]
    out = out.exp()
    out = out / (
...
class MultiHeadAttention2Layer(nn.Module):
    ...
    def propagate_attention(self, batch):
        graphgps = batch.K_h[batch.edge_index[0]]  # (num real edges) x num_heads x out_dim
        dest = batch.Q_h[batch.edge_index[1]]  # (num real edges) x num_heads x out_dim
        score = torch.mul(graphgps, dest)  # element-wise multiplication
        ...
        if self.full_graph:
            ...
            graphgps_2 = batch.K_2h[fake_edge_index[0]]  # (num fake edges) x num_heads x out_dim
            dest_2 = batch.Q_2h[fake_edge_index[1]]  # (num fake edges) x num_heads x out_dim
            score_2 = torch.mul(graphgps_2, dest_2)
