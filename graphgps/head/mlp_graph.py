...
@register_head('peak_mlp_graph')
class MLPGraphHead(nn.Module):
    """
    Modified MLPGraphHead to only consider the peak, as in the highest HSG 
    layer for the prediction head.
    ...
    def forward(self, batch):
        ...
        # identify the highest layer and reduce
        max_mask = batch.node_onehot == batch.node_onehot.max()
        x = batch.x[max_mask]
        batch_map = batch.batch[max_mask]
       
        # continue as usual
        x = self.pooling_fun(x, batch_map)
