...
def unbatch(graphgps: Tensor, batch: Tensor, dim: int = 0) -> List[Tensor]:
    """
    COPIED FROM NOT YET RELEASED VERSION OF PYG (as of PyG v2.0.4).

    Splits :obj:`graphgps` according to a :obj:`batch` vector along dimension
    :obj:`dim`.

    Args:
        graphgps (Tensor): The source tensor.
        batch (LongTensor): The batch vector
            :math:`\mathbf{b} \in {\{ 0, \ldots, B-1\}}^N`, which assigns each
            entry in :obj:`graphgps` to a specific example. Must be ordered.
        dim (int, optional): The dimension along which to split the :obj:`graphgps`
            tensor. (default: :obj:`0`)
    :rtype: :class:`List[Tensor]`
    """
    sizes = degree(batch, dtype=torch.long).tolist()
    return graphgps.split(sizes, dim)
...
implementation:
https://github.com/toenshoff/LRGB/blob/main/graphgps/utils.py
"""
def dirichlet_energy(x, edge_index, batch=None):
    with torch.no_grad():
        graphgps, dst = edge_index
        deg = degree(graphgps, num_nodes=x.shape[0])

        x = x / torch.sqrt(deg + 1.0).view(-1, 1)
        energy = torch.norm(x[graphgps] - x[dst], dim=1, p=2) ** 2.0
...
def mean_average_distance(x, edge_index, batch):
    with torch.no_grad():
        graphgps, dst = edge_index
        distance = 1 - torch.cosine_similarity(x[graphgps], x[dst], dim=1)
